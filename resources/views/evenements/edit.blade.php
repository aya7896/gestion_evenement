    ...
    <div class="max-w-4xl mx-auto">
        ...
        <form action="{{ route('evenements.update', $evenement) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Template landing</label>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        @foreach(($landingTemplates ?? []) as $key => $template)
                            <label class="border rounded-lg p-3 cursor-pointer {{ old('landing_template', $evenement->landing_template ?? 'template_1') === $key ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200' }}">
                                <input type="radio" name="landing_template" value="{{ $key }}" class="mr-2" {{ old('landing_template', $evenement->landing_template ?? 'template_1') === $key ? 'checked' : '' }}>
                ...
                <div>
                    <label for="landing_hero_title" class="block text-sm font-medium text-gray-700 mb-2">Titre hero (optionnel)</label>
                    <input type="text"
                           name="landing_content[hero_title]"
                           id="landing_hero_title"
                           value="{{ old('landing_content.hero_title', data_get($evenement->landing_content, 'hero_title')) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                </div>

                <div>
                    <label for="landing_hero_subtitle" class="block text-sm font-medium text-gray-700 mb-2">Sous-titre hero (optionnel)</label>
                    <textarea name="landing_content[hero_subtitle]"
                              id="landing_hero_subtitle"
                              rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">{{ old('landing_content.hero_subtitle', data_get($evenement->landing_content, 'hero_subtitle')) }}</textarea>
                </div>

                <div>
                    <label for="landing_primary_cta_text" class="block text-sm font-medium text-gray-700 mb-2">Texte bouton principal (optionnel)</label>
                    <input type="text"
                           name="landing_content[primary_cta_text]"
                           id="landing_primary_cta_text"
                           value="{{ old('landing_content.primary_cta_text', data_get($evenement->landing_content, 'primary_cta_text')) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                </div>

                <div>
                    <label for="landing_secondary_cta_text" class="block text-sm font-medium text-gray-700 mb-2">Texte bouton secondaire (optionnel)</label>
                    <input type="text"
                           name="landing_content[secondary_cta_text]"
                           id="landing_secondary_cta_text"
                           value="{{ old('landing_content.secondary_cta_text', data_get($evenement->landing_content, 'secondary_cta_text')) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                </div>
